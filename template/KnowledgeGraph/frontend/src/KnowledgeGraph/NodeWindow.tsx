...
const NodeWindow = ({ x, y, isVisible, nodeData, setWindowProps }: NodeWindowProps) => {
  ...
  const checkElementVisibility = (elem: HTMLElement) => !!elem && !!( elem.offsetWidth || elem.offsetHeight || elem.getClientRects().length )
  ...
  if (!nodeData) return <WindowContainer x={x} y={y} isVisible={isVisible} />

  return (
    // @ts-ignore
    <WindowContainer ref={windowRef} x={x} y={y} isVisible={isVisible}>
      <p><b>Title:</b> {nodeData.doc}</p>
      <b>Categories: </b>
      <ul>
        {map(nodeData.groups, (group) => (
          <li>
